...
namespace System.Formats.Asn1
{
    ...
    /// </summary>
    internal static partial class AsnDecoder
    {
            ...
            out int bytesConsumed)
        {
            ...
            if (localTag.IsConstructed)
            {
                throw new AsnContentException(
                    string.Format("The encoded value uses a constructed encoding, which is invalid for '{0}' values.", tagNumber));
            }
        ...
        private static int? ReadLength(ReadOnlySpan<byte> source, AsnEncodingRules ruleSet, out int bytesConsumed)
        {
            ...
            switch (status)
            {
                ...
                case LengthDecodeStatus.LengthTooBig:
                    throw new AsnContentException("The encoded length exceeds the maximum supported by this library (Int32.MaxValue).");
                case LengthDecodeStatus.LaxEncodingProhibited:
                case LengthDecodeStatus.DerIndefinite:
                    throw new AsnContentException("The encoded length is not valid under the requested encoding rules, the value may be valid under the BER encoding.");
        ...
        private static AsnContentException GetValidityException(LengthValidity validity)
        {
            ...
            switch (validity)
            {
                case LengthValidity.CerRequiresIndefinite:
                    return new AsnContentException("A constructed tag used a definite length encoding, which is invalid for CER data. The input may be encoded with BER or DER.");
                case LengthValidity.LengthExceedsInput:
                    return new AsnContentException("The encoded length exceeds the number of bytes remaining in the input buffer.");
        ...
        private static ReadOnlySpan<byte> Slice(ReadOnlySpan<byte> source, int offset, int length)
        {
            ...
            {
                throw new AsnContentException("The encoded length exceeds the number of bytes remaining in the input buffer.");
            }
    ...
    internal partial class AsnReader
    {
        ...
        public void ThrowIfNotEmpty()
        {
            if (HasData)
            {
                throw new AsnContentException("The last expected value has been read, but the reader still has pending data. This value may be from a newer schema, or is corrupt.");
            }
        }
