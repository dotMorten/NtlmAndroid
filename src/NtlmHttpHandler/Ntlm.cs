...
namespace System.Net
{
    ...
    internal class Ntlm
    {
        ...
        private byte[] makeNtlm2Hash(string domain, string userName, string password)
        {
            ...
            var md4 = new MD4();
            byte[] pwHash = new byte[md4.HashSizeBytes];
        ...
        {
#if DIAG
            Debug.WriteLine("NTLM challenge: {0}", challenge);
#endif
...
#if DIAG
            Debug.WriteLine("Negotiate challenege: {0} - {1} in {2}", challengeString, mech, state);
#endif
        ...
        private unsafe byte[] ProcessChallengeMessage(byte[] blob)
        {
            ...
            Debug.WriteLine("Get challenge from '{0}' with 0x{1:x} ({2}) flags", target, flags, FlagsEnumToString<Flags>(flags));
#endif
            ...
            if (targetInfo.Length > 0)
            {
                ...
                while (info.Length >= 4)
                {
                    ...
                    Debug.WriteLine("Got ID {0} with {1} len", ID, length);

                    if (ID == 0)
                    {
                        Debug.WriteLine("EOF on AV PAIRS reached!");
                        break;
                    }

                    if (4 + length > info.Length)
                    {
                        Debug.WriteLine("got field {0} with Len {1} while only {2} remaining", ID, length, info.Length);
                        break;
                    }
